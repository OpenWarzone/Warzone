...
bool dtCreateNavMeshData(dtNavMeshCreateParams* params, unsigned char** outData, int* outDataSize)
{
	...
	{
		printf("DT_VERTS_PER_POLYGON\n");
		return false;
	}
	if (params->vertCount >= 0xffff)
	{
		printf("vertCount too high.\n");
		return false;
	}
	if (!params->vertCount || !params->verts)
	{
		printf("vertCount 0 or verts == NULL\n");
		return false;
	}
	if (!params->polyCount || !params->polys)
	{
		printf("polyCount 0 or polys == NULL\n");
		return false;
	}
	...
	if (!data)
	{
		...
		printf("dtCreateNavMeshData: failed to allocate data.\n");
		return false;
	}
