...
NavMeshPathfind::NavMeshPathfind() :
	m_sample(0),
	m_navMesh(0),
	m_navQuery(0),
...
void NavMeshPathfind::init(Sample* sample)
{
	m_sample = sample;
...
void NavMeshPathfind::handleMenu()
{
#if 0
...
void NavMeshPathfind::handleAction(int mode, int straightPathOptions)
{
	if (mode == TOOLMODE_PATHFIND_FOLLOW)
	{
		m_toolMode = TOOLMODE_PATHFIND_FOLLOW;
		recalc();
	}
	if (mode == TOOLMODE_PATHFIND_STRAIGHT && straightPathOptions <= 0)
	{
		m_toolMode = TOOLMODE_PATHFIND_STRAIGHT;
		recalc();
	}
	if (mode == TOOLMODE_PATHFIND_STRAIGHT && straightPathOptions > 0)
	{
		if (straightPathOptions == 0)
		{
			m_straightPathOptions = 0;
			recalc();
		}
		if (straightPathOptions == DT_STRAIGHTPATH_AREA_CROSSINGS)
		{
			m_straightPathOptions = DT_STRAIGHTPATH_AREA_CROSSINGS;
			recalc();
		}
		if (straightPathOptions == DT_STRAIGHTPATH_ALL_CROSSINGS)
		{
			m_straightPathOptions = DT_STRAIGHTPATH_ALL_CROSSINGS;
			recalc();
		}
	}
	if (mode == TOOLMODE_PATHFIND_SLICED)
	{
		m_toolMode = TOOLMODE_PATHFIND_SLICED;
		recalc();
	}

	if (mode == TOOLMODE_DISTANCE_TO_WALL)
	{
		m_toolMode = TOOLMODE_DISTANCE_TO_WALL;
		recalc();
	}

	if (mode == TOOLMODE_RAYCAST)
	{
		m_toolMode = TOOLMODE_RAYCAST;
		recalc();
	}

	if (mode == TOOLMODE_FIND_POLYS_IN_CIRCLE)
	{
		m_toolMode = TOOLMODE_FIND_POLYS_IN_CIRCLE;
		recalc();
	}
	if (mode == TOOLMODE_FIND_POLYS_IN_SHAPE)
	{
		m_toolMode = TOOLMODE_FIND_POLYS_IN_SHAPE;
		recalc();
	}

	if (mode == TOOLMODE_FIND_LOCAL_NEIGHBOURHOOD)
	{
		m_toolMode = TOOLMODE_FIND_LOCAL_NEIGHBOURHOOD;
		recalc();
	}

	if (1)//imguiButton("Set Random Start"))
	{
		dtStatus status = m_navQuery->findRandomPoint(&m_filter, frand, &m_startRef, m_spos);
		if (dtStatusSucceed(status))
		{
	...
	if (1)// imguiButton("Set Random End", m_sposSet))
	{
		if (m_sposSet)
		{
			dtStatus status = m_navQuery->findRandomPointAroundCircle(m_startRef, m_spos, m_randomRadius, &m_filter, frand, &m_endRef, m_epos);
...
void NavMeshPathfind::handleClick(const float* /*s*/, const float* p, bool shift)
{
	if (shift)
	{
		m_sposSet = true;
...
void NavMeshPathfind::handleStep()
{
}

void NavMeshPathfind::handleToggle()
{
...
void NavMeshPathfind::handleUpdate(const float /*dt*/)
{
...
void NavMeshPathfind::handleRender()
{
#if 0
	DebugDrawGL dd;
...
void NavMeshPathfind::handleRenderOverlay(double* proj, double* model, int* view)
{
#if 0
	GLdouble x, y, z;
...
void NavMeshPathfind::drawAgent(const float* pos, float r, float h, float c, const unsigned int col)
{
#if 0
	DebugDrawGL dd;
