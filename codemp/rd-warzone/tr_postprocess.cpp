...
#endif //__SSDO__
	VectorSet4(vector, COLOR_GRADING_ENABLED, ssdoEnabled ? 1.0 : 0.0, 0.0, 0.0);
...
{
	shaderProgram_t *shader = &tr.ssrtgiBufferSetupShader;
...
*/
	...
	FBO_Blit(hdrFbo, NULL/*hdrBox*/, NULL, tr.ssrtgiBufferSetupFbo, NULL/*ldrBox*/, shader, colorWhite, 0);

	GL_Bind(tr.ssrtgiColorImage);
	qglGenerateMipmap(GL_TEXTURE_2D);
	GL_Bind(tr.ssrtgiDepthImage);
	qglGenerateMipmap(GL_TEXTURE_2D);
	GL_Bind(tr.ssrtgiNormalImage);
	qglGenerateMipmap(GL_TEXTURE_2D);
}

void RB_SSRTGI_StencilSetup(FBO_t *hdrFbo, vec4i_t hdrBox, FBO_t *ldrFbo, vec4i_t ldrBox)
{
	shaderProgram_t *shader = &tr.ssrtgiStencilSetupShader;
...
*/
	...
	if (shader->isBindless)
	{
		GLSL_SetBindlessTexture(shader, UNIFORM_DIFFUSEMAP, &tr.ssrtgiColorImage, 0);
		GLSL_SetBindlessTexture(shader, UNIFORM_SCREENDEPTHMAP, &tr.linearDepthImageZfar, 0);
		GLSL_SetBindlessTexture(shader, UNIFORM_NORMALMAP, &tr.ssrtgiNormalImage, 0);
		GLSL_BindlessUpdate(shader);
	}
	else
	{
		GLSL_SetUniformInt(shader, UNIFORM_DIFFUSEMAP, TB_DIFFUSEMAP);
		GL_BindToTMU(tr.ssrtgiColorImage, TB_DIFFUSEMAP);

		GLSL_SetUniformInt(shader, UNIFORM_SCREENDEPTHMAP, TB_LIGHTMAP);
		GL_BindToTMU(tr.linearDepthImageZfar, TB_LIGHTMAP);

		GLSL_SetUniformInt(shader, UNIFORM_NORMALMAP, TB_NORMALMAP);
		GL_BindToTMU(tr.ssrtgiNormalImage, TB_NORMALMAP);
	}
	...
	FBO_Blit(hdrFbo, NULL/*hdrBox*/, NULL, tr.ssrtgiStencilSetupFbo, NULL/*ldrBox*/, shader, colorWhite, 0);
}

void RB_SSRTGI_RayTrace(FBO_t *hdrFbo, vec4i_t hdrBox, FBO_t *ldrFbo, vec4i_t ldrBox)
{
	shaderProgram_t *shader = &tr.ssrtgiRayTraceShader;
...
*/
	...
	if (shader->isBindless)
	{
		...
		GLSL_SetBindlessTexture(shader, UNIFORM_SCREENDEPTHMAP, &tr.linearDepthImageZfar, 0);
		GLSL_SetBindlessTexture(shader, UNIFORM_NORMALMAP, &tr.ssrtgiNormalImage, 0);
		GLSL_SetBindlessTexture(shader, UNIFORM_STEEPMAP, &tr.ssrtgiGIPrevImage, 0);
		GLSL_SetBindlessTexture(shader, UNIFORM_STEEPMAP1, &tr.ssrtgiGBufferPrevImage, 0);
		GLSL_SetBindlessTexture(shader, UNIFORM_STEEPMAP2, &tr.ssrtgiJitterImage, 0);
		GLSL_SetBindlessTexture(shader, UNIFORM_STEEPMAP3, &tr.ssrtgiDepthImage, 0);
		GLSL_BindlessUpdate(shader);
	}
	else
	{
		...
		GL_BindToTMU(tr.ssrtgiGIPrevImage, TB_STEEPMAP);

		GLSL_SetUniformInt(shader, UNIFORM_STEEPMAP1, TB_STEEPMAP1);
		GL_BindToTMU(tr.ssrtgiGBufferPrevImage, TB_STEEPMAP1);

		GLSL_SetUniformInt(shader, UNIFORM_STEEPMAP2, TB_STEEPMAP2);
		GL_BindToTMU(tr.ssrtgiJitterImage, TB_STEEPMAP2);

		GLSL_SetUniformInt(shader, UNIFORM_STEEPMAP3, TB_STEEPMAP3);
		GL_BindToTMU(tr.ssrtgiDepthImage, TB_STEEPMAP3);
	}
	...
	FBO_Blit(hdrFbo, NULL/*hdrBox*/, NULL, tr.ssrtgiRayTraceFbo, NULL/*ldrBox*/, shader, colorWhite, 0);

	GL_Bind(tr.ssrtgiGlobalIllminationImage);
	qglGenerateMipmap(GL_TEXTURE_2D);
}

void RB_SSRTGI_CopyFilter(FBO_t *hdrFbo, vec4i_t hdrBox, FBO_t *ldrFbo, vec4i_t ldrBox)
{
	shaderProgram_t *shader = &tr.ssrtgiCopyFilterShader;

	GLSL_BindProgram(shader);

/*
uniform sampler2D								u_DiffuseMap;		// sSSRTGI_ColorTex
...
*/
	...
	{
		GLSL_SetBindlessTexture(shader, UNIFORM_DIFFUSEMAP, &tr.ssrtgiColorImage, 0);
		GLSL_SetBindlessTexture(shader, UNIFORM_STEEPMAP, &tr.ssrtgiGlobalIllminationImage, 0);
		GLSL_SetBindlessTexture(shader, UNIFORM_STEEPMAP1, &tr.ssrtgiGBufferImage, 0);
		GLSL_BindlessUpdate(shader);
	}
	else
	{
		...
		GLSL_SetUniformInt(shader, UNIFORM_STEEPMAP, TB_STEEPMAP);
		GL_BindToTMU(tr.ssrtgiGlobalIllminationImage, TB_STEEPMAP);

		GLSL_SetUniformInt(shader, UNIFORM_STEEPMAP1, TB_STEEPMAP1);
		GL_BindToTMU(tr.ssrtgiGBufferImage, TB_STEEPMAP1);
	}
	...
	FBO_Blit(hdrFbo, NULL/*hdrBox*/, NULL, tr.ssrtgiCopyFilterFbo, NULL/*ldrBox*/, shader, colorWhite, 0);
}

void RB_SSRTGI_Output(FBO_t *hdrFbo, vec4i_t hdrBox, FBO_t *ldrFbo, vec4i_t ldrBox)
{
	shaderProgram_t *shader = &tr.ssrtgiOutputShader;
...
*/
	...
	if (shader->isBindless)
	{
		...
		GLSL_SetBindlessTexture(shader, UNIFORM_STEEPMAP, &tr.ssrtgiGIPrevImage, 0);
		GLSL_SetBindlessTexture(shader, UNIFORM_STEEPMAP1, &tr.ssrtgiGBufferImage, 0);
		GLSL_SetBindlessTexture(shader, UNIFORM_STEEPMAP2, &tr.ssrtgiDepthImage, 0);
		GLSL_BindlessUpdate(shader);
	}
	else
	{
		...
		GLSL_SetUniformInt(shader, UNIFORM_STEEPMAP2, TB_STEEPMAP2);
		GL_BindToTMU(tr.ssrtgiDepthImage, TB_STEEPMAP2);
	}
	...
	FBO_Blit(hdrFbo, NULL/*hdrBox*/, NULL, ldrFbo, NULL/*ldrBox*/, shader, colorWhite, 0);
}
#endif //__SSRTGI__
