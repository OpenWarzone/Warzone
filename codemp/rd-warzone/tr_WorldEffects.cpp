...
*/
	...
	void		Render()
	{
		...
		// For light effects...
		//
		extern int					NUM_CURRENT_DRAW_DLIGHTS; // set up by deferredlighting from previous frame...
		...
		float						CURRENT_DRAW_DLIGHTS_EMISSIVE_DISTANCE[MAX_DEFERRED_LIGHTS];

		for (int i = 0; i < NUM_CURRENT_DRAW_DLIGHTS; i++)
		{
			CURRENT_DRAW_DLIGHTS_EMISSIVE_DISTANCE[i] = Distance(backEnd.refdef.vieworg, CURRENT_DRAW_DLIGHTS_POSITIONS[i]);
		}
