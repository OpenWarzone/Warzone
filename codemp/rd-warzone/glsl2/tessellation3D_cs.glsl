...
out float GrassSlope_ES_in[MAX_PATCH_VERTICES];
...
void main()
{
	...
	GrassSlope_ES_in[gl_InvocationID] = GrassSlope_CS_in[gl_InvocationID];
...
#define START_LOD_RANGE u_Local9.g//4096.0
	...
	if (dist > START_LOD_RANGE)
	{// Closer then 4096.0 gets full tess...
		distFactor = (dist-START_LOD_RANGE) / START_LOD_RANGE;
	}*/
...
#else
	// LODs, tessellate less the further from the camera this vert is...
	float dist1 = round(distance(Vert1, u_ViewOrigin));
	float dist2 = round(distance(Vert2, u_ViewOrigin));
	float dist3 = round(distance(Vert3, u_ViewOrigin));

	float distFactor1 = dist1 > START_LOD_RANGE ? (dist1-START_LOD_RANGE) / START_LOD_RANGE : 1.0;
	float distFactor2 = dist2 > START_LOD_RANGE ? (dist2-START_LOD_RANGE) / START_LOD_RANGE : 1.0;
	float distFactor3 = dist3 > START_LOD_RANGE ? (dist3-START_LOD_RANGE) / START_LOD_RANGE : 1.0;

	float uTessLevel1 = max(uTessLevel1 / distFactor1, 1.0);
	float uTessLevel2 = max(uTessLevel2 / distFactor2, 1.0);
	float uTessLevel3 = max(uTessLevel3 / distFactor3, 1.0);

	gl_TessLevelOuter[0] = uTessLevel1;
	gl_TessLevelOuter[1] = uTessLevel2;
	gl_TessLevelOuter[2] = uTessLevel3;

	gl_TessLevelInner[0] = max(uTessLevel1, max(uTessLevel2, uTessLevel3));
#endif
}
