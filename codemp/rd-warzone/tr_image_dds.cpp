...
void R_LoadDDS ( const char *filename, byte **pic, int *width, int *height, GLenum *picFormat, int *numMips )
{
	...
	*pic = (byte *)Z_Malloc(len, TAG_IMAGE_T, qfalse, 0);
	...
	ri->Printf(PRINT_ALL, "DDS File %s loaded. Format %s. Size %i x %i. Mips %i.\n", filename, format, *width, *height, *numMips);
}

// Loads a DDS image from file.
void LoadDDS(const char *filename, byte **data, int *width, int *height)
{
	GLenum picFormat = GL_RGBA8;
	int numMips = 1;
	R_LoadDDS(filename, data, width, height, &picFormat, &numMips);
}
