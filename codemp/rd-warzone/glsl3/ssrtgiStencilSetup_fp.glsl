...
void main()
{
	...
	float linearDepth		= linear_depth(v.texcoord.xy);

	if (linearDepth >= max(RT_FADE_DEPTH.x, RT_FADE_DEPTH.y))
	{// theoretically only .y but users might swap it...
		gl_FragColor = vec4(0.0);
		return;
	}
	
	gl_FragColor = vec4(tex2D(u_NormalMap, v.texcoord_scaled.xy).xyz * 2.0 - 1.0, depth2dist(linearDepth) / DEPTH_LINEARIZATION_FAR_PLANE);
}
