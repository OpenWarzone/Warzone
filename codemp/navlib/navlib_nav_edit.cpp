...
void Cmd_NavEdit()
{
	...
	{
		trap->Print( "navedit only available in local devmap\n" );
		return;
	}
	...
	if ( !Q_stricmp( arg, "enable" ) )
	{
		...
		if ( i == numNavData )
		{
			trap->Print( "\'%s\' is not a valid navmesh name\n", arg );
			return;
		}
...
void Cmd_AddConnection()
{
	...
	if ( !Q_stricmp( arg, "start" ) )
	{
		...
		{
			trap->Print( "Invalid argument for direction, specify oneway or twoway\n" );
			return;
		}
...
static void adjustConnectionSize( int dir )
{
	...
	if ( argc > 1 )
	{
		adjust = Q_clamp( atoi( Cmd_Argv( 1 ) ), 1, 20 );
	}

	newConnectionSize = Q_clamp( connectionSize + dir * adjust, 20, 100 );

	if ( newConnectionSize != connectionSize )
	{
		connectionSize = newConnectionSize;
		trap->Print( "Default connection size = %d\n", connectionSize );
	}
}
...
void Cmd_NavTest()
{
	...
	if ( !cmd.enabled )
	{
		trap->Print( "%s", "Can't test navmesh without enabling navedit\n" );
		return;
	}
