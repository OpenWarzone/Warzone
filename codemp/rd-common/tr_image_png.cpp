...
int RE_SavePNG( const char *filename, byte *buf, size_t width, size_t height, int byteDepth ) {
	...
	*/
	...
	png_set_IHDR (png_ptr,
		...
		height,
		depth,
		(byteDepth == 4) ? PNG_COLOR_TYPE_RGBA : PNG_COLOR_TYPE_RGB,
		PNG_INTERLACE_NONE,
...
int RE_SavePNG_Flipped(const char *filename, byte *buf, size_t width, size_t height, int byteDepth) {
	fileHandle_t fp;
	png_structp png_ptr = NULL;
	png_infop info_ptr = NULL;
	unsigned int x, y;
	...
	*/
	...
	for (y = 0; y < height; ++y) {
		...
		row_pointers[height - y - 1] = row;
		for (x = 0; x < width; ++x) {
			byte *px = buf + ((width * (height-y-1)) + x)*byteDepth;
			*row++ = px[0];
			*row++ = px[1];
			*row++ = px[2];
			if (byteDepth == 4) *row++ = px[3];
